...
The service worker coordinates scans, forwards ingest requests from content scripts, updates run logs and extension state on the backend, and closes the scan window (popup) when a run finishes. **Auto-scrape** (optional): `background/auto_scrape.js` runs **pre-check** probes (session + **CAPTCHA** URL/body heuristics), **`runOneCycle`** matrix scans, merges state via **`PUT /admin/auto-scrape/state`**, and schedules **`auto_scrape_next_cycle`** alarms; `poll.js` mirrors backend control flags, sends heartbeats when enabled, **self-bootstraps** the next-cycle alarm, and can trigger **graceful exit** on shutdown. **`notifications`** permission + `icons/icon128.png` drive per-site **CAPTCHA** desktop notifications (click opens the probe URL). **`auto_scrape_init.js`** registers instance id and notification click handler. See repo root **README** (Auto-scrape) and `PHASE_5_NOTES.md`.
...
│   ├── poll.js                      Poll **`GET /extension/pending`** (~30s alarm) + mirror **`/admin/auto-scrape/state`**, heartbeat, **self-bootstrap** next-cycle alarm
│   ├── auto_scrape_config.js        Probe URLs + timing constants (`AS_PROBE_URLS`, …)
│   ├── auto_scrape.js               Pre-check, probe, matrix, **`runOneCycle`**, **`onAutoScrapeAlarm`**, CAPTCHA **notifications**, state merge helpers
│   ├── auto_scrape_init.js         Init `_autoScrape`; **alarm** + **notification.onClicked** listeners
