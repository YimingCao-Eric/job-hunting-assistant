...
The service worker coordinates scans, forwards ingest requests from content scripts, updates run logs and extension state on the backend, and closes the scan window (popup) when a run finishes. **Auto-scrape** (optional): `background/auto_scrape.js` runs **`runOneCycle`** — on entry it immediately **`PUT`**s **`cycle_phase: scrape_running`** (parallel-cycle guard before pre-check), then **pre-check** probes (session + **CAPTCHA** URL/body heuristics; bare **403** without captcha markers → **rate_limited**), fetches **`GET /admin/auto-scrape/config`** for **`enabled_sites`** × **`keywords`** each cycle (falls back to **`AS_DEFAULT_*`** in **`auto_scrape_config.js`** if needed), runs the matrix, merges state via **`PUT /admin/auto-scrape/state`**, and resets **`cycle_phase`** to **`idle`** in **`finally`**. **`handleGracefulExit`** closes leftover **job-board popup** windows (LinkedIn / Indeed / Glassdoor URLs), clears alarms/storage, and merges idle orchestrator state. **`_closeScrapePopupWindows`** is also invoked from **`auto_scrape_init.js`** after init to remove orphan popups when no cycle is active. `poll.js` mirrors backend control flags, sends heartbeats when enabled, **self-bootstraps** `auto_scrape_next_cycle` only when **`cycle_phase`** is not **`scrape_running`** / **`postscrape_running`**, and can trigger **graceful exit** on shutdown. **`notifications`** permission + `icons/icon128.png` drive per-site **CAPTCHA** desktop notifications (click opens the probe URL). **`auto_scrape_init.js`** registers instance id, orphan-popup cleanup, alarm handler, and notification **onClicked**. See repo root **README** (Auto-scrape) and `PHASE_5_NOTES.md`.

**Scan debug trace:** content scripts use **`JhaDebug`** (`content/shared/debug_logger.js`) to record structured events (pagination, cards, session checks, etc.) in **`chrome.storage.local.debugLog`**; the background worker flushes batches to **`POST /extension/run-log/{runId}/debug`** and sends a final flush when the run completes (**`scan_completion.js`**). The web app **Logs** page can show and export this stream per run. **Dedup** runs on the server (`POST /jobs/dedup` or automatic after run completion when `dedup_mode` is sync). Post-scan dedup is scheduled with **`asyncio.create_task`** so closing the extension tab does not cancel it. **Matching** (CPU/LLM job-description extraction, gates, CPU scoring, optional LLM re-score) is **web-app only**: open the **Matching** page in the JHA UI (`/matching`), which calls `POST /jobs/match` with modes such as `cpu_only`, `llm_extraction_gates`, `cpu_score`, or `llm_score` (queued on the server as a detached task) and polls match reports — the extension does not invoke those endpoints. **Issue reports** (flag on job cards → `POST /jobs/{id}/report`) and the **Logs → Reports** tab are also **web-app only**. The **popup** (`popup/popup.html`) lets the user set the backend URL, trigger **Scan Now** (`MANUAL_SCAN`), stop a scan, and see live progress read from storage. Full config (including `website`, `indeed_*`, nested **`glassdoor`**, **`dedup_mode`**, **`llm`**) is edited on the **web Config page** or via `PUT /config` — see repo root `README.md`. Run history, dedup reports, pipeline match reports, and issue reports live in the **web app** (**Logs**); the extension does not render those UIs.
...
│   ├── poll.js                      Poll **`GET /extension/pending`** (~30s alarm) + mirror **`/admin/auto-scrape/state`**, heartbeat, **self-bootstrap** next-cycle alarm (uses **`state.cycle_phase`**, not `scanInProgress`)
│   ├── auto_scrape_config.js        Probe URLs + timing constants (`AS_PROBE_URLS`, **`AS_DEFAULT_SITES`**, **`AS_DEFAULT_KEYWORDS`**, …)
│   ├── auto_scrape.js               Pre-check, probe, backend orchestrator **config** fetch, matrix, **`runOneCycle`**, **`handleGracefulExit`**, **`onAutoScrapeAlarm`**, CAPTCHA **notifications**, popup cleanup, state merge helpers
│   ├── auto_scrape_init.js          Init `_autoScrape`; **alarm** + **notification.onClicked**; orphan **scrape popup** cleanup after persist
...
**Manifest permissions** include **`windows`** (with **`tabs`**, **`storage`**, **`scripting`**, **`activeTab`**, **`alarms`**, **`notifications`**) so the background script can open the search URL in a **separate popup window** via `chrome.windows.create`, remove the scan tab by id on completion or force-stop, and show **CAPTCHA** alerts tied to auto-scrape.
