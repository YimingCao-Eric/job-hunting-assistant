...
- **Per-source ingest (`routers/jobs.py`):** **`build_linkedin_params`**, **`build_indeed_params`**, and **`build_glassdoor_params`** normalize types before raw SQL inserts because **asyncpg** does not coerce Python values across mismatched column kinds. Use **`_to_str_or_none`** when JSON/API payloads deliver numbers but the DB column is **`VARCHAR`/`TEXT`** (e.g. LinkedIn **`job_posting_id`**). Use **`_to_int_or_none`** when **`INTEGER`** columns receive numeric strings (e.g. Glassdoor JSON-LD **`experienceRequirements.monthsOfExperience`**, **`gocId`**, **`jobCountryId`**; defensive wraps on Indeed **`location_count`**, **`apply_count`**, **`num_hires`**). Leave **`NUMERIC`** salary fields unwrapped. See **`LINKEDIN_COLS`**, **`INDEED_COLS`**, **`GLASSDOOR_COLS`** for column order vs. params dict keys.
